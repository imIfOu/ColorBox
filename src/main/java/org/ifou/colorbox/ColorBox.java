...
public class ColorBox extends HBox {
	...
	public void layoutChildren() {
		...
		if (stage.getMinWidth() > 0 && stage.getMinHeight() > 0) {

			return;
		}
		stage.setMinWidth(computeMinWidth(getHeight()));
		stage.setMinHeight(computeMinHeight(getWidth()));
	}
...
private class ColorRectPane extends HBox {
	...
	private DoubleProperty hue=new SimpleDoubleProperty(0){@Override protected void invalidated(){if(!changeIsLocal){changeIsLocal=true;updateHSBColor();changeIsLocal=false;}}};
	private DoubleProperty sat=new SimpleDoubleProperty(0){@Override protected void invalidated(){if(!changeIsLocal){changeIsLocal=true;updateHSBColor();changeIsLocal=false;}}};
	private DoubleProperty bright=new SimpleDoubleProperty(0){@Override protected void invalidated(){if(!changeIsLocal){changeIsLocal=true;updateHSBColor();changeIsLocal=false;}}};
	private IntegerProperty red=new SimpleIntegerProperty(0){@Override protected void invalidated(){if(!changeIsLocal){changeIsLocal=true;updateRGBColor();changeIsLocal=false;}}};

	private IntegerProperty green=new SimpleIntegerProperty(0){@Override protected void invalidated(){if(!changeIsLocal){changeIsLocal=true;updateRGBColor();changeIsLocal=false;}}};

	private IntegerProperty blue=new SimpleIntegerProperty(0){@Override protected void invalidated(){if(!changeIsLocal){changeIsLocal=true;updateRGBColor();changeIsLocal=false;}}};

	private DoubleProperty alpha=new SimpleDoubleProperty(100){@Override protected void invalidated(){if(!changeIsLocal){changeIsLocal=true;setCustomColor(new Color(getCustomColor().getRed(),getCustomColor().getGreen(),getCustomColor().getBlue(),clamp(alpha.get()/100)));changeIsLocal=false;}}};
...
private class ColorChecker extends VBox implements MouseMoveListener {
	...
	private ChangeListener<Boolean> focuschange = new ChangeListener<Boolean>() {
			public void changed(ObservableValue<? extends Boolean> ov, Boolean t,Boolean t1){stopAction();}};

	private EventHandler<Event> eventMouse=new EventHandler<Event>(){@Override public void handle(Event event){stopAction();}};
